...
unsigned int LiteralDictionary::stringToId(std::string &key,
	...
	case SUBJECT:
		...
		ret = subjects->locate((const unsigned char *) key.c_str(),
				key.length());
		if (ret != 0) {
	...
	case PREDICATE:
		ret = predicates->locate((const unsigned char *) key.c_str(),
				key.length());
		if (ret != 0) {
	...
	case OBJECT:
		if (key.at(0) == '"') {
			...
			ret = shared->locate((const unsigned char *) key.c_str(),
					key.length());
			if (ret != 0) {
				return getGlobalId(ret, SHARED_OBJECT);
			}
			ret = objectsNotLiterals->locate(
